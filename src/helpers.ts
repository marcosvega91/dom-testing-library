import {Callback, Nullish} from './types'
...
function runWithRealTimers<T>(callback: Callback<T>) {
...
function getJestFakeTimersType() {
  ...
  if ('_isMockFunction2' in globalObj.setTimeout) {
    return 'legacy'
  }

  if (
    'clock' in globalObj.setTimeout &&
...
function setImmediatePolyfill(fn: Callback<void>) {
...
function getWindowFromNode(node: Document | Node | Window) {
  if ('defaultView' in node) {
    ...
    node.ownerDocument &&
    'defaultView' in node.ownerDocument
  ) {
    // node is a DOM node
    ...
    'then' in node &&
    ((node as unknown) as Promise<unknown>).then instanceof Function
  ) {
    throw new Error(
...
function checkContainerType(container: Nullish<Element>) {
  if (
    !container ||
